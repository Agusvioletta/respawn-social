...
type Section = 'perfil' | 'banners' | 'conexiones' | 'privacidad' | 'notificaciones' | 'seguridad' | 'cuenta' | 'logros' | 'faq'
...
export default function SettingsPage() {
  ...
  // Conexiones
  const [connections, setConnections] = useState({ steamId: '', discordUsername: '', twitchUsername: '' })
  ...
  useEffect(() => {
    ...
    setConnections({
      ...
      twitchUsername:  u.twitch_username   ?? '',
    })
    loadStats()
  ...
  async function handleSaveConnections() {
    ...
    try {
      const updates = {
        steam_id:         connections.steamId.trim()         || null,
        discord_username: connections.discordUsername.trim() || null,
        twitch_username:  connections.twitchUsername.trim()  || null,
      }
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const { error } = await (supabase as any).from('profiles').update(updates).eq('id', user.id)
      if (error) throw error
      setUser({ ...user, ...updates } as typeof user)
      setConnMsg('✓ Conexiones guardadas.')
    } catch (e: unknown) {
      setConnMsg('⚠ ' + (e instanceof Error ? e.message : 'Error'))
    } finally {
      setSavingConn(false)
    }
  }
    ...
    <div style={{ maxWidth: '640px', margin: '0 auto', padding: '24px 16px' }}>
      ...
      {section === 'conexiones' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <div style={cardStyle}>
            <h3 style={{ fontFamily: 'var(--font-display)', fontSize: '13px', fontWeight: 700, color: 'var(--text-primary)', letterSpacing: '1px', marginBottom: '8px' }}>🔗 Cuentas externas</h3>
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', color: 'var(--text-muted)', marginBottom: '20px' }}>
              // Vinculá tus cuentas de gaming. Se muestran en tu perfil público.
            </p>
            ...
            <div style={{ marginBottom: '20px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                ...
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', color: '#66C0F4', letterSpacing: '1px' }}>STEAM</span>
              </div>
              <input
                type="text"
                value={connections.steamId}
                onChange={e => setConnections(c => ({ ...c, steamId: e.target.value }))}
                placeholder="ID numérico o vanity URL (ej: GabeNewell)"
                style={inputStyle}
              />
              <p style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', color: 'var(--text-muted)', marginTop: '5px' }}>
                // Encontralo en steamcommunity.com/id/<strong style={{ color: 'var(--text-secondary)' }}>tuvanity</strong> o en tu perfil Steam → Editar perfil
              </p>
            </div>
            ...
            <div style={{ marginBottom: '20px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                ...
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', color: '#5865F2', letterSpacing: '1px' }}>DISCORD</span>
              </div>
              <input
                type="text"
                value={connections.discordUsername}
                onChange={e => setConnections(c => ({ ...c, discordUsername: e.target.value }))}
                placeholder="usuario#0000 o nombre de usuario nuevo"
                style={inputStyle}
              />
            </div>
            ...
            <div style={{ marginBottom: '24px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                ...
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', color: '#9146FF', letterSpacing: '1px' }}>TWITCH</span>
              </div>
              <input
                type="text"
                value={connections.twitchUsername}
                onChange={e => setConnections(c => ({ ...c, twitchUsername: e.target.value }))}
                placeholder="tu usuario de Twitch"
                style={inputStyle}
              />
            </div>

            {saveBtn(handleSaveConnections, savingConn, '💾 GUARDAR CONEXIONES')}
            {connMsg && (
              <p style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', color: connMsg.startsWith('✓') ? 'var(--cyan)' : 'var(--pink)', marginTop: '8px' }}>
                {connMsg}
              </p>
            )}
          </div>
